...
if (isset($_POST['login'])) {
    ...
    if ($email == "" or $password == "") {
        ...
        <font color='red'>Please enter email or password!</font>
    </div>";
    } else {
        $sql = "SELECT * FROM users where email='$email'";
        $val = mysqli_query($con,$sql);
        $r = mysqli_fetch_object($val);
        $uname = $r->name;
        $uemail = $r->email;
        $upass = $r->password;
        if ($uemail == $_POST['email'] && $upass == $_POST['password']) {
            $_SESSION['name'] = $uname;
            $_SESSION['email'] = $_POST['email'];
            header('location:index.php');
        } else {
            $err = "<div class='alert alert-warning'>
            <font color='red'>Please enter correct email or password!</font>
        </div>";
        }
    }
}
...
<body>
    <header class="w-100 d-flex flex-row bg-primary justify-content-between shadow-lg">
        ...
        <div class='profile w-25 d-none d-lg-flex justify-content-center align-items-center position-relative'>
            <a href="#" class="text-decoration-none"><img class="profile-img" src=<?php echo 'profile/'.$img ?> alt=""></a>
